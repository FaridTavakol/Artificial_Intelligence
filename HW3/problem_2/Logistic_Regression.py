# -*- coding: utf-8 -*-
"""
Logistic Regression with Regularization Class
"""
import numpy as np
...
class LogisticRegression(object):
    ...
    Parameters:
    1) learningRate : float, optional
    A constant by which updates are multiplied (falls between 0 and 1). Defaults to 0.01.

    2) numEpochs : int, optional
    Number of Iterations over the training data.(Default = 10)

    3) penalty : None or 'L2'
    Option to perform RidgeRegression(L2) regularization. Defaults to None.

    Attributes:
    1) weights : 1d-array, shape = [1, 1 + n_features]
    Weights after training phase

    2) iterationsPerformed : int
    Number of iterations of gradient descent performed prior to hitting tolerance level

    3) costs : list, length = numEpochs
    Value of the log-likelihood cost function for each iteration of gradient descent
    """

    def __init__(self, learningRate, numEpochs=10, penalty=None, C=0.01):
    ...
    def train(self, X_train, y_train, tol=10 ** -4):
        ...
        Parameters:
        1) X_train : {array-like}, shape = [n_samples, n_features]
        Training dataset used for estimating the model constants, where n_samples is the number of 
        samples and n_features is the number of features. 

        2) y_train : array-like, shape = [n_samples,], values = 1|0
        Actual output value labels (target values). 
        ...
        for i in range(self.numEpochs):
            ...
            if self.penalty is not None:
                delta_w = self.learningRate * \
                    (self.C * np.dot(errors, X_train) + np.sum(self.weights))
            else:
            ...
            if np.all(abs(delta_w) >= tolerance):
                ...
                # Costs
                if self.penalty is not None:
                    self.costs.append(reg_logLiklihood(
                        X_train, self.weights, y_train, self.C))
                else:
    ...
    def predict(self, X_test, pi=0.5):
        ...
        Parameters:
        1) X_test : {array-like}, shape = [n_samples, n_features]
    ...
    def performanceEval(self, predictions, y_test):
        ...
        2) y_test : list, shape = [n_samples,], values = 1|0
        True class labels
